...
2. **Build.** The CLI fetches the licence key for the project's Project ID from the OctoberCMS API, generates a temporary `auth.json` in a secure temp directory, and runs `docker build` with `DOCKER_BUILDKIT=1` and `--secret id=composer_auth,src=<temp file>`. The temp file is deleted immediately after the build completes (in a `begin/ensure` block to guarantee cleanup even on failure). The licence key is in memory only briefly; it never enters image layers, the build cache, or any logged output.
...
2. **Init.** `octobercms init` calls the OctoberCMS API to list the customer's Projects, presents them as a selectable list, and lets the customer either pick an existing Project or create a new one inline. The selected Project's ID is written to `.kamal/project` (a non-secret file safe to commit).
3. **Build.** At build time, the CLI fetches the licence key for the project's Project ID via the API (using the stored account token), generates a temporary `auth.json` file in Composer's expected format, and passes it to `docker build` via `--mount=type=secret,id=composer_auth,target=/app/auth.json`. The temp file is deleted immediately after the build completes (in a `begin/ensure` block to guarantee cleanup even on failure).
